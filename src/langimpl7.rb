...
module Toy
  ...
  def self.gettok
    ...
    if isalpha $LastChar # identifier: [a-zA-Z][a-zA-Z0-9]*
      ...
      return TOK_VAR if $IdentifierStr == 'var' 
  ...
  class VarExprAST < ExprAST
    attr_accessor :var_names, :body
    def initialize var_names, body
      self.var_names, self.body = var_names, body
    end

    def to_s
      "VarExprAST(#{@var_names}, #{@body})"
    end
  end
  ...
  def self.ParseParenExpr
    ...
    return Error("expected ')': #{$CurTok}") unless $CurTok == ')'
    get_next_token # eat ).
    v
  end
  ...
  def self.ParseVarExpr()
    get_next_token  # eat the var.

    var_names = []

    # At least one variable name is required.
    return Error("expected identifier after var: #{$CurTok}") unless $CurTok == TOK_IDENTIFIER
    ...
    loop do
      ...
      return Error("expected identifier list after var: #{$CurTok}") unless $CurTok == TOK_IDENTIFIER
    end

    # At this point, we have to have 'in'.
    return Error("expected 'in' keyword after 'var': #{$CurTok}") unless $CurTok == TOK_IN
    get_next_token  # eat 'in'.

    body = ParseExpression()
    return 0 if body == 0

    VarExprAST.new var_names, body
  end
  ...
  class ::Object
    def CreateEntryBlockAlloca the_function, var_name
      pointer = nil
      the_function.basic_blocks.entry.build do |b|
        pointer = b.alloca LLVM::Double, var_name.is_a?(String) ? var_name : var_name.name
      end
      pointer
    end
  end
  ...
  class VariableExprAST
    def codegen
      ...
      v = $NamedValues[@name]
      return ErrorV("Unknown variable name: #{@name}") if v == 0
  ...
  class BinaryExprAST
    def codegen
      ...
      if @op == '='
        ...
        return ErrorV("destination of '=' must be a variable: #{@lhs}") unless @lhs
        # Codegen the RHS.
        val = @rhs.codegen
        return 0 if val == 0
        ...
        return ErrorV("Unknown variable name: #{@lhs.name}") if variable == 0

        $Builder.store val, variable
        return val
      end
  ...
  class ForExprAST
    def codegen
      ...
      alloca = CreateEntryBlockAlloca(the_function, @var_name)
      ...
      $Builder.store start_val, alloca
      ...
      cur_var = $Builder.load alloca, @var_name
      next_var = $Builder.fadd cur_var, step_val, 'nextvar'
  ...
  class VarExprAST
    def codegen
      ...
      #@var_names.reverse.each do |var_name, init|
      @var_names.reverse.each do |var_name, init|
        ...
        if init != 0
          ...
          return 0 if init_val == 0
        else # If not specified, use 0.0.
          init_val = LLVM::Double.from_f 0.0
        end
        ...
        # we unrecurse.
        old_bindings << $NamedValues[var_name]
      ...
      body_val = @body.codegen
      return 0 if body_val == 0

      # Pop all our variables from scope.
      @var_names.each_with_index do |var_name, i|
        $NamedValues[var_name.first] = old_bindings[i]
      end
  ...
  class PrototypeAST
    def create_argument_allocas f
      f.params.each do |arg|
        # Create an alloca for this variable.
        alloca = CreateEntryBlockAlloca(f, arg)
